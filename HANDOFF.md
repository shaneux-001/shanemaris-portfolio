...
- **New finding, not yet resolved — live hard-boundary violation:** Heart Design System Chapter 2 (`app/work/heart-design-system/chapter-2/page.tsx`) names a real person, "Vince Bratton," violating the never-name-colleagues rule. This is pre-9/3 content (restyled for Tailwind in Phase 6, prose never touched/audited). Needs Shane's decision on how to anonymize.
- **Broader sweep requested by Shane (2026-09-05), not started:** per his instruction, nothing edited before 9/3 should be trusted without review — this includes all 4 HDS chapters (beyond the Vince Bratton finding), the 10 hidden case-study `.md` files in `content/work/`, and the 5 resume experience-entry bullets not touched this session (Lead UX Designer, Sr. UX Designer, UX Designer, Web Designer, UX Designer/Contractor) — these already read materially differently from the new master MD in several places and need a side-by-side comparison.
