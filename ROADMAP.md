...
## Done — "The Recipe Box" → Labs

**Shipped 2026-09-19.** Built on the `labs/recipe-box` branch (off the `v1.0.0` tag), merged into `main` via PR #1, and confirmed live: `/labs` lists it, `/labs/recipe-box` correctly redirects unauthenticated visitors to its login page, and a full production build passes with the new `@upstash/ratelimit` + `@upstash/redis` dependencies installed.
...
**Scope confirmed 2026-09-19 — supersedes the prompt below on one key point:** the AI planning is a live **in-app chat feature**, not Shane chatting with Claude in a separate conversation. Concretely: a server-side Route Handler calls the Claude API (key never shipped to the client), gated behind a signed-cookie session (not the site's existing `NEXT_PUBLIC_*` client-side password pattern — real API cost is at stake here) with Upstash-backed rate limiting as a backstop against bugs/abuse. The recipe library, meal-pairing heuristic, shopping-list builder, and pantry-diffing logic from the prototype were reused/ported (`lib/labs/recipeBox/`); the AI's job is limited to picking/pairing meals from the existing library (returning structured `{main, side, reason}` per meal) and taking free-text "what's in the fridge" input — the deterministic JS still does the ingredient math. v1 shipped single-shot (describe fridge → get a plan → refine), not a full multi-turn chat thread. Pantry state persists via Upstash, keyed to the session, so it's available from any device. Auth is `proxy.ts` (Next 16's rename of `middleware.ts`) plus a signed, httpOnly session cookie re-verified independently inside each API route — not relying on the proxy layer alone, per Next's own guidance that it shouldn't be a full authorization solution by itself.
