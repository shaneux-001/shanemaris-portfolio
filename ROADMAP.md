...
- [ ] **Accessibility audit (WCAG 2.1 AA).** May roadmap claims this was done — focus-visible ring, contact form label associations, tag-pill contrast, Safari focus-ring fix. All of that was against the OLD design system (old tag pills, old `.project-card` classes) which no longer exists — the entire UI was rebuilt in the September Press Room redesign. None of those specific fixes necessarily carried over. Needs a fresh audit against the current site. **One thing already found** during the 2026-09-07 responsive pass, worth folding into that audit rather than a one-off fix: several tap targets measure under the 44px recommended minimum at 375px — nav links (32px tall), the theme toggle (30px tall), the contact form's submit button (43px tall). Widths are generous on all of them; only height is short.
...
- [ ] **`/labs` has no page-specific `<title>`/meta description** — found during the 2026-09-07 responsive pass. `/contact` gets its metadata from a sibling `app/contact/layout.tsx` (since its `page.tsx` is a client component and can't export `metadata` directly); `/labs` has no equivalent `layout.tsx`, so it just inherits the root layout's default title/description. Small SEO/polish gap, not a layout bug — fix by adding `app/labs/layout.tsx` following the `/contact` pattern.
