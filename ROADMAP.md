...
- [ ] **Accessibility audit (WCAG 2.1 AA).** May roadmap claims this was done — focus-visible ring, contact form label associations, tag-pill contrast, Safari focus-ring fix. All of that was against the OLD design system (old tag pills, old `.project-card` classes) which no longer exists — the entire UI was rebuilt in the September Press Room redesign. None of those specific fixes necessarily carried over. Needs a fresh audit against the current site. ~~One thing already found during the responsive pass: several tap targets under 44px~~ — fixed 2026-09-07, commit `cd34d62` (nav links, theme toggle, all `.pr-cta`/`.pr-btn-secondary` buttons bumped to 44px+ height). Rest of the audit still needed.
