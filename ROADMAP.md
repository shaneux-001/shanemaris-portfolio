...
- [x] ~~Performance / Lighthouse pass~~ — `next/image` migration done 2026-09-14, commit `ad7e5c5`. Added `lib/imageDimensions.ts` (PNG-header reader, no new dependency) since every image lives in `public/` with no build-time asset pipeline; verified via network requests that Next's optimizer is actually resizing/converting images. **Not yet Lighthouse-verified** — no Lighthouse tool available in the session that did this work; Shane should run it in Chrome DevTools to confirm the actual score before calling this fully closed.
